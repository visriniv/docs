...
Koala Data Explorer Extension provides a powerful results viewer that helps you explore, analyze, and understand your Oracle Fusion Cloud data. This guide covers all features available for viewing and interacting with query results.
...
- Collapse/expand using the chevron in the results header
...
1. Click inside the grid and press `Ctrl+F` (Windows/Linux) or `Cmd+F` (Mac).
2. A search bar appears above the grid; the first match is highlighted.
3. Use the arrows in the search bar or `Enter` / `Shift+Enter` to move between matches.

<Tip>
  In-grid search respects current sorting and filters, so narrow results first for faster navigation.
</Tip>
...
- **Auto column width**: Use the toolbar option to fit all columns to current data
