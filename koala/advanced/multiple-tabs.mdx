...
Koala Data Explorer Extension supports multiple query tabs, allowing you to work on several SQL queries simultaneously. This feature improves productivity by letting you switch between different queries without losing your work.
...
### Tab isolation and switching

- Each tab runs queries in its own execution context; cancelling one tab does not stop others.
- You can switch tabs while a query runs in the background, then return to monitor progress or cancel.
- Tabs remember their active connection and results independently.
...
- Right-click a file-backed tab and choose **Reveal in Explorer** to open its folder.
- Hover over a tab title to see the full file name and path.
- Improved tab sizing keeps more of the file name visible when many tabs are open.
